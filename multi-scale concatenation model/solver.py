...
class Solver(object):
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            series, prior = self.model(input, pool_sizes=defined_pool_sizes)
            ...
            for u in range(len(prior)):  # 遍历 prior
                if self.win_size != prior[u].shape[-1]:
                    temp_win_size = prior[u].shape[-1]
                    series_loss += (torch.mean(my_kl_loss(series[u], (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, temp_win_size)).detach())) + torch.mean(my_kl_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, temp_win_size)).detach(), series[u])))  # 累加 series 损失
                    prior_loss += (torch.mean(my_kl_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, temp_win_size)), series[u].detach())) + torch.mean(my_kl_loss(series[u].detach(), (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, temp_win_size)))))  # 累加 prior 损失
                else:
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):  # 遍历所有 epochs
            ...
            print("Batch structure:", type(sample_batch))
            print("Number of elements in batch:", len(sample_batch))
            print("Shapes:", [x.shape if hasattr(x, 'shape') else x for x in sample_batch])'''
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.train_loader):  # 遍历训练数据加载器
            input = input_data.float().to(self.device)  # 将输入数据转换为 float 并移动到设备上
            #series, prior = self.model(input)  # 获取模型输出
            series, prior = self.model(input, pool_sizes=defined_pool_sizes)  # 获取模型输出
            series_loss = 0.0  # 初始化 series 损失
            prior_loss = 0.0  # 初始化 prior 损失
            for u in range(len(prior)):  # 遍历 prior

                '''print("len", len(prior))
                print(f"series[{u}] shape:", series[u].shape)
                print(f"prior[{u}] shape:", prior[u].shape)'''
                ...
                # 此处为新添加
                elif self.win_size != prior[u].shape[-1]:
                    temp_win_size = prior[u].shape[-1]
                    # self.win_size = prior[u].shape[-1]
                    temp_series_loss = my_kl_loss(series[u], (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, temp_win_size)).detach()) * temperature  # 累加 series 损失
                    new_length = (temp_series_loss.size(1) // self.win_size) * self.win_size
                    if new_length == 0:
                        pad_size = self.win_size - temp_series_loss.size(1)
                        padded_loss = F.pad(temp_series_loss, (0, pad_size), "constant", 0)  # 右侧补零
                        series_loss += padded_loss
                    else:
                        times = new_length // self.win_size
                        for i in range(times):
                            series_loss += temp_series_loss[:, (self.win_size*i):(self.win_size*(i+1))]

                    temp_prior_loss = my_kl_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, temp_win_size)), series[u].detach()) * temperature  # 累加 prior 损失
                    new_length = (temp_prior_loss.size(1) // self.win_size) * self.win_size
                    if new_length == 0:
                        pad_size = self.win_size - temp_prior_loss.size(1)
                        padded_loss = F.pad(temp_prior_loss, (0, pad_size), "constant", 0)  # 右侧补零
                        prior_loss += padded_loss
                    else:
                        times = new_length // self.win_size
                        for i in range(times):
                            prior_loss += temp_prior_loss[:, (self.win_size * i):(self.win_size * (i + 1))]
                else:
        ...
        for i, (input_data, labels) in enumerate(self.thre_loader):  # 遍历阈值数据加载器
            ...
            series_loss = 0.0  # 初始化 series 损失
            prior_loss = 0.0  # 初始化 prior 损失
            for u in range(len(prior)):  # 遍历 prio
                if u == 0:  # 如果是第一个 prior
                    series_loss = my_kl_loss(series[u], (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, self.win_size)).detach()) * temperature  # 计算 series 损失
                    prior_loss = my_kl_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1, self.win_size)), series[u].detach()) * temperature  # 计算 prior 损失
                    # 此处为新添加
