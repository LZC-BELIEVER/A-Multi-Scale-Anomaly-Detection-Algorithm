...
class DAC_structure(nn.Module):
    ...
    def forward(self, queries_patch_size, queries_patch_num, keys_patch_size, keys_patch_num, values, patch_index, attn_mask):
        ...
        '''print("ori_size shape before", series_patch_size.shape)
        print("ori_num shape before", series_patch_num.shape)'''
        ...
        if patch_index == -1:
            ...
            temp, temp, size_H, temp = series_patch_size.shape
            temp, temp, num_H, temp = series_patch_num.shape
            lcm = abs(size_H * num_H) // math.gcd(size_H, num_H)
            size_repeat_num = lcm // size_H
            num_repeat_num = lcm // num_H
            #print("lcm, size_H, num_H, size_repeat_num, num_repeat_num", lcm, size_H, num_H, size_repeat_num, num_repeat_num)
            '''print("repeat_num_0", repeat_num_0)
            repeat_num = size_H // num_H
            print("repeat num", repeat_num)'''
            series_patch_size = repeat(series_patch_size, 'b l m n -> b l (m repeat_m) (n repeat_n)', repeat_m=size_repeat_num, repeat_n=size_repeat_num)
            series_patch_num = series_patch_num.repeat(1, 1, num_repeat_num, num_repeat_num)

            '''print("ori_size shape middle", series_patch_size.shape)
            print("ori_num shape middle", series_patch_num.shape)'''
