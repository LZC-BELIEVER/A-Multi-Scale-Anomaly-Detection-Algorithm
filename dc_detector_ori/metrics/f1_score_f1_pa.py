...
def get_adjust_F1PA(pred, gt):
    ...
    with open('./pred.txt', 'w') as f:
        for i in range(0,len(gt)):
            if(gt[i]!=0):
                print(f"gt:{gt[i]},index{i}")
            if(pred[i]!=0):
                print(f"pred:{pred[i]},index{i}")
                line = f"{i}\n"
                f.write(line)
    count=0
    ...
    with open('../result.txt', 'w') as f:
                line="Number:{:0.1f},Accuracy : {:0.4f}, Precision : {:0.4f}, Recall : {:0.4f}, F-score : {:0.4f} \n".format(count,accuracy, precision, recall, f_score)
                f.write(line)
    '''
